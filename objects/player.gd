...
var health: int = 100
var signal_strength: int = 100
var energy: int = 80
var gravity := 0.0
var signal_decay_rate: float = 0.3
...
func _process(delta):
	...
	# Signal decay + energy regen
	var old_signal := signal_strength
	signal_strength = maxi(0, signal_strength - int(signal_decay_rate * delta))
	if signal_strength != old_signal:
		signal_updated.emit(signal_strength)
	if signal_strength <= 0:
		get_tree().reload_current_scene()
	energy = mini(100, energy + int(2 * delta))
...
func damage(amount):
	...
	# Also take some signal damage
	signal_strength = maxi(0, signal_strength - int(amount * 0.3))
	signal_updated.emit(signal_strength)
